...
instructions_multilang = {
    'English': {
        'line1': "Connect the circles in order as quickly as possible.",
        'line2_order': "The order is:",
        'line3_continue': "Click on the green circle to continue."
    },
    'Русский': {
        'line1': "Соединяйте круги по порядку как можно быстрее.",
        'line2_order': "Порядок следующий:",
        'line3_continue': "Нажмите на зелёный круг, чтобы продолжить."
    },
    'Қазақша': {
        'line1': "Шеңберлерді ретімен мүмкіндігінше жылдам қосыңыз.",
        'line2_order': "Реті:",
        'line3_continue': "Жалғастыру үшін жасыл шеңберді басыңыз."
    }
}
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instr_parts = instructions_multilang[chosen_lang]
    ...
    instrText = visual.TextStim(win=win, name='instrText',
        text='',
        font='Arial',
        pos=(0, 0.3), draggable=False, height=0.05, wrapWidth=1.5, ori=0, 
    ...
    for thisTrial in trials:
        ...
        # --- ИЗМЕНЕНИЕ: Формируем инструкцию из частей на выбранном языке ---
        final_instr_text = f"{instr_parts['line1']}\n{instr_parts['line2_order']}\n{msg}\n\n{instr_parts['line3_continue']}"
        instrText.setText(final_instr_text)
        ...
        while continueRoutine:
            ...
            # Run 'Each Frame' code from trialCode
            # Обновляем позицию нашего видимого курсора
            trialCursor.setPos(trialMouse.getPos())

            # Проверяем, была ли ТОЛЬКО ЧТО нажата левая кнопка мыши
            if trialMouse.getPressed()[0] == 1 and prevButtonState[0] == 0:
                # Регистрируем клик
                
                # Проверяем, попал ли клик в СЛЕДУЮЩУЮ ПРАВИЛЬНУЮ цель
                if trialTargets[trialStep].contains(trialMouse):
                    # УСПЕШНЫЙ КЛИК
                    # 1. Меняем цвет цели на зеленый
                    trialText[trialStep].setColor(green)
                    
                    # 2. Рисуем линию, если это не первая цель
                    if trialStep > 0:
                        line = visual.ShapeStim(
                            win=win, name="line"+str(trialStep),
                            lineColor=white, lineWidth=3,
                            vertices=[trialTargets[trialStep-1].pos, trialTargets[trialStep].pos]
                        )
                        shapeList.append(line)
                    
                    # 3. Переходим к следующей цели
                    trialStep += 1
                    
                else: 
                    # ОШИБОЧНЫЙ КЛИК (клик был, но не по правильной цели)
                    # 1. Увеличиваем счетчик ошибок
                    # (добавь errorCount = 0 в начале рутины 'trial', если еще не добавил)
                    try:
                        errorCount += 1
                    except NameError:
                        errorCount = 1
                    
                    # 2. Даем обратную связь - вспышка красным
                    win.color = 'red' 
                    # Перерисовываем все, чтобы ничего не пропало
                    ...
                    win.flip()
                    core.wait(0.1)
                    win.color = [-.67,-.67,-.67] # Возвращаем фон

            # Обновляем состояние кнопки для следующего кадра
            prevButtonState = trialMouse.getPressed()

            # --- УСЛОВИЕ ЗАВЕРШЕНИЯ ---
            if trialStep == len(targetList):
                latency = routineTimer.getTime() # Используем таймер рутины для замера времени
                thisExp.addData('Score', round(latency, 2))
                # Сохраняем результаты
                if 'A' in Condition:
                    trial_results['A']['latency'] = round(latency, 2)
                    try: 
                        trial_results['A']['errors'] = errorCount
                    except NameError:
                         trial_results['A']['errors'] = 0
                elif 'B' in Condition:
                    trial_results['B']['latency'] = round(latency, 2)
                    try:
                        trial_results['B']['errors'] = errorCount
                    except NameError:
                        trial_results['B']['errors'] = 0
                continueRoutine = False
    ...
    # --- Add this before endExperiment ---
# Calculate and save the ratio
    # --- Add this before endExperiment ---
    
        # --- Add this before endExperiment ---
    if trial_results['A']['latency'] is not None and trial_results['B']['latency'] is not None:
        trial_results['ratio'] = trial_results['B']['latency'] / trial_results['A']['latency']
    
    # Add trial_results to the experiment data
    thisExp.addData('Trial_A_latency', trial_results['A']['latency'])
    thisExp.addData('Trial_B_latency', trial_results['B']['latency'])
    thisExp.addData('Ratio_B_over_A', trial_results['ratio'])
    
    # Save to a separate JSON file
    import json
    results_filename = os.path.join(_thisDir, 'data', f"{expInfo['participant']}_trail_making_results.json")
    with open(results_filename, 'w') as f:
        json.dump(trial_results, f, indent=4)
    
    # Save to a separate CSV file
    import csv
    csv_filename = os.path.join(_thisDir, 'data', f"{expInfo['participant']}_trail_making_results.csv")
    with open(csv_filename, 'w', newline='') as csvfile:
        fieldnames = ['Trial_A_latency', 'Trial_B_latency', 'Ratio_B_over_A']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        writer.writeheader()
        writer.writerow({
            'Trial_A_latency': trial_results['A']['latency'],
            'Trial_B_latency': trial_results['B']['latency'],
            'Ratio_B_over_A': trial_results['ratio']
        })
