...
# ADD THESE GLOBAL VARIABLES (right after imports, before showExpInfoDlg function):
current_rule = None
prev_rule = None
rule_changed = False
perseverative_errors = 0
trial_data = []  # To store trial-by-trial data for the new CSV

# --- ИЗМЕНЕНИЕ: Словарь для инструкций на трех языках ---
instructions_multilang = {
    'English': {
        'intro': "In this task you will be required to sort the presented cards based on a rule i.e. the cards will have to be sorted based on either colour, shape or number. The rule will not be presented, however you will receive feedback on each trial. After a certain amount of trials the rule will change to a different one. To select your response click on one of the four cards presented at the top of the screen.",
        'example_intro': "For example, you may see a screen like this:",
        'example_desc': "The presented card (bottom centre) can be categorised based on shape (dots), colour (yellow) or number (three). Click on the first card if you want to categorise it by the shape, second if you want categorise it by colour and third if you want to categorise it by number. After you select your response, feedback will be provided. Click on the image to start the experiment.",
        'feedback_correct': "Correct!",
        'feedback_incorrect': "Incorrect",
        'end_message': "This is the end of the experiment.\nThank you for your time."
    },
    'Русский': {
        'intro': "В этом задании вам нужно будет сортировать представленные карточки согласно определенному правилу: по цвету, форме или количеству. Правило не будет показано, но после каждой попытки вы получите обратную связь. После определенного количества попыток правило изменится. Для выбора ответа нажмите на одну из четырех карточек в верхней части экрана.",
        'example_intro': "Например, вы можете увидеть такой экран:",
        'example_desc': "Представленную карту (внизу в центре) можно классифицировать по форме (точки), цвету (желтый) или количеству (три). Нажмите на первую карту, если хотите классифицировать по форме, на вторую — по цвету, на третью — по количеству. После вашего ответа будет предоставлена обратная связь. Нажмите на изображение, чтобы начать эксперимент.",
        'feedback_correct': "Правильно!",
        'feedback_incorrect': "Неправильно",
        'end_message': "Это конец эксперимента.\nСпасибо за ваше время."
    },
    'Қазақша': {
        'intro': "Бұл тапсырмада сізге көрсетілетін карталарды белгілі бір ереже бойынша сұрыптау қажет: түсіне, пішініне немесе санына қарай. Ереже көрсетілмейді, бірақ әр әрекеттен кейін кері байланыс аласыз. Белгілі бір әрекеттен кейін ереже өзгереді. Жауапты таңдау үшін экранның жоғарғы жағындағы төрт картаның бірін басыңыз.",
        'example_intro': "Мысалы, сіз осындай экранды көруіңіз мүмкін:",
        'example_desc': "Көрсетілген картаны (төменгі ортада) пішіні (нүктелер), түсі (сары) немесе саны (үш) бойынша жіктеуге болады. Егер пішіні бойынша жіктегіңіз келсе, бірінші картаны, түсі бойынша — екіншіні, саны бойынша — үшіншіні басыңыз. Жауабыңыздан кейін кері байланыс беріледі. Экспериментті бастау үшін суретке басыңыз.",
        'feedback_correct': "Дұрыс!",
        'feedback_incorrect': "Қате",
        'end_message': "Бұл эксперименттің соңы.\nУақытыңыз үшін рахмет."
    }
}
...
expInfo = {
    'participant': '',
    'session': '001',
    # --- ИЗМЕНЕНИЕ: Добавлен выбор языка в стартовое окно ---
    'language': ['English', 'Русский', 'Қазақша'],
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- ИЗМЕНЕНИЕ: Получаем выбранный язык и части инструкций ---
    chosen_lang = expInfo.get('language', 'English') 
    lang_texts = instructions_multilang[chosen_lang]

    
    # --- Initialize components for Routine "Instructions" ---
    instructions = visual.TextStim(win=win, name='instructions',
        text=lang_texts['intro'], # ИЗМЕНЕНИЕ: Используем текст из словаря
        font='Arial',
    ...
    example_text = visual.TextStim(win=win, name='example_text',
        text=lang_texts['example_intro'], # ИЗМЕНЕНИЕ: Используем текст из словаря
        font='Arial',
    ...
    example_text_2 = visual.TextStim(win=win, name='example_text_2',
        text=lang_texts['example_desc'], # ИЗМЕНЕНИЕ: Используем текст из словаря
        font='Arial',
    ...
    thank_you = visual.TextStim(win=win, name='thank_you',
        text=lang_texts['end_message'], # ИЗМЕНЕНИЕ: Используем текст из словаря
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0, 
        color='white', colorSpace='rgb', opacity=1, 
        languageStyle='LTR',
    ...
    for thisBlock in blocks:
        ...
        for thisTrial in trials:
            ...
            trial_card.setImage(card)
                        ...
                        # Track rule changes
            global current_rule, prev_rule, rule_changed, perseverative_errors, trial_data
            ...
            if corr == 1:
                msg = lang_texts['feedback_correct']
            else:
                msg = lang_texts['feedback_incorrect']
            feedback_text.setText(msg)
            
                        # Track perseverative errors
            global perseverative_errors, trial_data
            if corr == 0 and rule_changed and prev_rule is not None:
            ...
            trial_data.append({
                ...
                'block': blocks.thisN + 1,
                'rule': current_rule,
                'response': response.clicked_name[0] if len(response.clicked_name) else None,
                'correct_response': corrAns,
                'correct': corr,
                'rt': rt,
                'rule_changed': rule_changed,
                'perseverative_error': is_perseverative
            })
...
def save_perseverative_data(thisExp, trial_data):
    """
    Save perseverative error data to a separate CSV file.
    """
    # Get the base filename from the experiment
    base_filename = thisExp.dataFileName
    
    # Create filenames
    pe_filename = os.path.join(
        os.path.dirname(base_filename),
        f"perseverative_errors_{os.path.basename(base_filename)}.csv"
    )
    summary_filename = os.path.join(
        os.path.dirname(base_filename),
        f"perseverative_summary_{os.path.basename(base_filename)}.csv"
    )
    
    # Save trial data
    df = pd.DataFrame(trial_data)
    df.to_csv(pe_filename, index=False)
    
    # Save summary
    summary = {
        'total_perseverative_errors': df['perseverative_error'].sum(),
        'total_errors': len(df) - df['correct'].sum(),
        'participant': thisExp.extraInfo['participant'],
        'date': thisExp.extraInfo['date']
    }
...
def endExperiment(thisExp, win=None):
    ...
    if win is not None:
        ...
        win.flip()
    # Save perseverative error data
    save_perseverative_data(thisExp, trial_data)
