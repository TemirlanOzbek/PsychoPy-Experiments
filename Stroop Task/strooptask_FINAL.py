...
def runExperiment():
    ...
    for trial in trials:
        ...
        wordStim.color = COLORS[trial['letterColor']]  # Works with any language
        ...
        rt = None
        while resp is None:
            keys = event.getKeys(keyList=list(TRANSLATIONS[lang]['keys'].keys()) + ['escape'], timeStamped=globalClock)
            if keys:
                ...
                valid_keys = [k for k in keys if k[0] in TRANSLATIONS[lang]['keys'].keys()]
                if valid_keys:
                    resp, rt = valid_keys[0]
                    rt = rt - trialStart
        ...
        trial_data = {
            ...
            'response': resp,
            'correct': correct,
            'rt': rt * 1000 if rt else None,  # convert to ms
        }
...
def save_data(results):
    ...
    summary = {
        ...
        'mean_rt_congruent': congruent_valid['rt'].mean(),
        'mean_rt_incongruent': incongruent_valid['rt'].mean(),
        'stroop_acc_cost': incongruent['correct'].mean() - congruent['correct'].mean(),
        'stroop_rt_cost': incongruent_valid['rt'].mean() - congruent_valid['rt'].mean(),
    }
