...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Desktop\\corsi_test\\Experiment\\corsi_blocks_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    """
    english_text = 'Welcome to the Corsi Blocks Test!\n\n\n TASK DETAILS: \n You will see a sequence of blocks light up in a specific order \n Your task is to click on the blocks in the SAME ORDER they were shown. \n- The test starts with 3 blocks\n- If you get the sequence correct, the next trial will show 1 more block\n- If you make a mistake, the sequence length stays the same\n- The test continues until you make 3 mistakes at a given sequence length\n\nClick the "Submit" button after selecting all blocks in order.\n\nPress any key or click to begin the test.'
    kazakh_text = kazakh_text = "Corsi blocks тестіне қош келдіңіз!\nТАПСЫРМА НҰСҚАУЛЫҒЫ:\nСіз белгілі бір ретпен жанатын блоктардың тізбегін көресіз. Сіздің міндетіңіз — блоктарды КӨРСЕТІЛГЕН РЕТТЕ басу.\n- Тест 3 блоктан басталады - Егер дұрыс орындасаңыз, келесі раундта 1 блок қосылады\n- Қателессеңіз, тізбек ұзындығы өзгермейді - Тест берілген ұзындықта 3 қате жібергенше жалғасады\nБарлық блоктарды ретімен таңдаған соң Submit түймесін басыңыз. Тестті бастау үшін кез келген пернені басыңыз."
    russian_text = russian_text = "Добро пожаловать в тест Corsi blocks!\nДЕТАЛИ ЗАДАНИЯ: Вы увидите последовательность подсвечивающихся блоков в определенном порядке. Ваша задача — кликнуть на блоки в ТОМ ЖЕ ПОРЯДКЕ, в котором они были показаны.\n- Тест начинается с 3 блоков - Если вы правильно воспроизведете последовательность, в следующем испытании будет на 1 блок больше - Если вы допустите ошибку, длина последовательности останется прежней - Тест продолжается, пока вы не совершите 3 ошибки на данной длине последовательности Нажмите кнопку Submit после выбора всех блоков по порядку. Нажмите любую клавишу или кликните, чтобы начать тест."

    text = visual.TextStim(win=win, name='text',
            text='Welcome to the Corsi Blocks Test!\n\n\n TASK DETAILS: \n You will see a sequence of blocks light up in a specific order \n Your task is to click on the blocks in the SAME ORDER they were shown. \n- The test starts with 3 blocks\n- If you get the sequence correct, the next trial will show 1 more block\n- If you make a mistake, the sequence length stays the same\n- The test continues until you make 3 mistakes at a given sequence length\n\nClick the "Submit" button after selecting all blocks in order.\n\nPress any key or click to begin the test.',
            font='Arial',
    ...
    # --- Set instructions based on selected language (ADD THESE LINES) ---
    if expInfo['language'] == 'English':
        text.text = english_text
    elif expInfo['language'] == 'Kazakh':
        text.text = kazakh_text
    elif expInfo['language'] == 'Russian':
        text.text = russian_text
    ...
    # Create instruction texts for each language (ADD THESE LINES)
    ...
    for thisCorsi_task in corsi_task:
        ...
        if block_sequence.trialList in ([], [None], None):
            params = []
        else:
            params = block_sequence.trialList[0].keys()
        # save data for this loop
        block_sequence.saveAsExcel(filename + '.xlsx', sheetName='block_sequence',
            stimOut=params,
        ...
        # Reset the clicked names list at start of each trial
        corsi_mouse.clicked_name = []
        ...
        # Only proceed if exactly nBlocks were clicked
        if len(order_clicked) == nBlocks:
            # create a list to store accuracy
            correct_list = []
            count = 0
            for name in order_clicked:
                if count < len(correct_order) and name == correct_order[count]:
            ...
            # if all clicks were correct
            if sum(correct_list) == nBlocks:
                correct = 1
        else:
            # If wrong number of blocks were clicked, count as incorrect
            correct_list = [0]*nBlocks
            thisExp.addData('correct_list', correct_list)
        ...
        # if the sequence clicked was correct
        if expInfo['language'] == 'English':
            thisfb = 'Correct!' if correct else 'Incorrect!'
            fbcol = 'green' if correct else 'red'
        elif expInfo['language'] == 'Kazakh':
            thisfb = 'Дұрыс!' if correct else 'Қате!'
            fbcol = 'green' if correct else 'red'
        elif expInfo['language'] == 'Russian':
            thisfb = 'Правильно!' if correct else 'Неправильно!'
            fbcol = 'green' if correct else 'red'
        ...
        if not correct:
            ...
            # Reset incorrect count if they get it right
            incorrect_count = 0

        if incorrect_count >= 3:  # Changed to >= for safety
    ...
    if thisSession is not None:
        ...
        thisSession.sendExperimentData()
    # get names of stimulus parameters
    if corsi_task.trialList in ([], [None], None):
        params = []
    else:
        params = corsi_task.trialList[0].keys()
    # save data for this loop
    corsi_task.saveAsExcel(filename + '.xlsx', sheetName='corsi_task',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    ...
    continueRoutine = True
    # update component parameters for each repeat
    # Update the end message based on language
    if expInfo['language'] == 'English':
        end_msg = f'Your maximum blocks remembered was: {nBlocks-1}'
    elif expInfo['language'] == 'Kazakh':
        end_msg = f'Есіңізде қалған блоктардың максималды саны: {nBlocks-1}'
    elif expInfo['language'] == 'Russian':
        end_msg = f'Максимальное количество запомненных блоков: {nBlocks-1}'

    text_2.setText(end_msg)
    ...
    if expInfo['language'] == 'English':
        results_data = {
            ...
            'Max. blocks remembered': [str(nBlocks-1)]
        }
    elif expInfo['language'] == 'Kazakh':
        results_data = {
            'Қатысушы': [expInfo['participant']],
            'Есіңізде қалған блоктардың макс. саны': [str(nBlocks-1)]
        }
    elif expInfo['language'] == 'Russian':
        results_data = {
            'Участник': [expInfo['participant']],
            'Макс. количество запомненных блоков': [str(nBlocks-1)]
        }
    ...
    # Save to Excel
    output_filename = f"corsi_blocks_results_{expInfo['participant']}.xlsx"
    ...
    # create keyboard component for the end routine
